...
int normal_case(unsigned int key_buf[], unsigned int data_buf[], unsigned int res[])
{
	...
	u32 res1[4];
	getRK(key_buf, K, RK);
		encryptSM4(data_buf, RK, Y);
		//printf("************** software compute *****************\n");
		decryptSM4(Y, RK, res);
...
int nice_case(unsigned int key_buf[], unsigned int data_buf[] , unsigned int res[])
{
 ...
 /* custom_lkey((int)key_buf);
  ...
  //printf("************** hardware compute *****************\n");
  custom_lkey((int)key_buf);
  custom_ldata((int)data_buf);
  decrypt();
  custom_sm4_res((int)res);
  //printf("%X %X %X %X", init_buf3[0], init_buf3[1], init_buf3[2], init_buf3[3]); printf("\n");
