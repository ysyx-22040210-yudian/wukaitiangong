...
module e203_biu(
 ...
 assign arbt_bus_icb_cmd_usr =
  ...
  .o_icb_rsp_usr          (1'b0   )     ,
    //将IFU与LSU的总线接口信号包装成一组总线信号的输入，用以仲裁器的仲裁                           
...
//    guranteed by performance. If IFU really suppose to be replayed, then good luck to break this deadlock.
  ...
  //当有外设访问时，且当前的仲裁器给出的访问地址在外设的地址范围内时，将外设的访问请求转发到外设的接口
  wire buf_icb_cmd_ppi = ppi_icb_enable & (buf_icb_cmd_addr[`E203_PPI_BASE_REGION] ==  ppi_region_indic[`E203_PPI_BASE_REGION]);
  wire buf_icb_sel_ppi = buf_icb_cmd_ppi & (~buf_icb_cmd_ifu); //当前没有IFU占用BIU的时候，才能转发给PPI
  ...
  /*
  "Dont allow different branches outstanding"（不允许不同的分支未完成）是一种硬件设计或通信协议中的限制或规则。

在硬件设计中，当多个分支或处理器同时访问共享资源时，可能会引发冲突或数据一致性问题。为了避免这种问题，可以设置一些规则来限制同时访问的分支或处理器的数量。 "Don't allow different branches outstanding" 就是这样一种规则，它指示系统在同一时间不允许多个不同的分支或处理器同时访问共享资源。

这种限制通常可以通过硬件设计中的控制信号、状态机、互斥锁等方式来实现，以确保在同一时间只有一个分支或处理器可以访问共享资源，从而避免冲突和数据一致性问题。这种限制对于多处理器系统、并行计算、多总线系统等应用中的并发访问管理非常重要。*/
