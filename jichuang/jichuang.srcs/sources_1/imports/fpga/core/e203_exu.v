...
module e203_exu(
  ...
  input  [`E203_PC_SIZE-1:0] i_pc,   // The PC register along with ***************As the bpu_fixpc_i***************
  //****************new start BPU***************************//
  input [`BHR_WIDTH-1:0]        i_BHR,
  input [1:0]                   i_PHT,
  input [`E203_PC_SIZE-1:0]     i_bpu_taken_pc,
  input                         i_bpu_taken,
  //****************new end BPU***************************//
  //************************new start************************ *************
  output [`BHR_WIDTH-1:0]     bjp_o_cmt_bhr     ,// The predicted ture/false
  output [1:0]                bpu_fixpht_w      ,// The predicted ture/false
  output [`E203_PC_SIZE-1:0]  bpu_fixpc         ,// As the bpu_fixpc_iaa
  output [`E203_PC_SIZE-1:0]  bpu_fixjumpaddr   ,// As the bpu_fixjumpaddr_i
  output                      bjp_o_cmt_taken   ,// The predicted ture/false
  output                        cmt_taken     ,// ex_isjump              

  output [2:0]                ex_bjop_o         ,
  output                      bpu_fixwe_w       ,
  output                      btb_fixwe_w       ,
  ...
  //**************************new oitf******************************///
  //********************************new start******************************
  output [`E203_OITF_DEPTH-1:0]   vld_r     , //
  output [`E203_OITF_DEPTH-1:0]   rdwen_r,
  output [`E203_RFIDX_WIDTH-1:0] rdidx_r0,
  output [`E203_RFIDX_WIDTH-1:0] rdidx_r1,
  output [`E203_RFIDX_WIDTH-1:0] rdidx_r2,
  output [`E203_RFIDX_WIDTH-1:0] rdidx_r3,
//********************************new end *********************************--
  //***************************new end **********************************//
  input  i_pc_vld,
...
wire dec_jalr, dec_jal, dec_bxx, dec_bjp;
...
wire prmiss, prsuccess, cmt_isjump ;
wire [1:0] bjp_o_cmt_pht;
wire [`E203_PC_SIZE-1:0]  bjp_o_cmt_takenpc;
wire [`E203_PC_SIZE-1:0] prdt_pc_add_op1;  
wire [`E203_PC_SIZE-1:0] prdt_pc_add_op2;
wire [`E203_XLEN-1:0] dec_rs1;
wire [`E203_PC_SIZE-1:0] jump_addr;
  ...
  e203_exu_disp u_e203_exu_disp(
    ...
    .disp_i_rs1idx       (i_rs1idx      ), //仅仅用于作为oitf的rs1idx
    .disp_i_rs2idx       (i_rs2idx      ), //仅仅用于作为oitf的rs2idx
    .disp_i_rdwen        (dec_rdwen       ),
    .disp_i_rdidx        (dec_rdidx       ),  //仅仅用于作为oitf的rdidx
...
//********************new end********************************//
  ...
  `endif//}
    ...
    //*******************************new start*****************************
      .bjp_o_cmt_bhr        (bjp_o_cmt_bhr    ),
      .bjp_o_cmt_pht        (bjp_o_cmt_pht    ),
      .bjp_o_cmt_takenpc    (bjp_o_cmt_takenpc    ),
      .bjp_o_cmt_taken      (bjp_o_cmt_taken    ),
    //*******************************new end*******************************
    .i_pc                (i_pc    ),
    .i_pc_vld            (i_pc_vld),
    .i_instr             (i_ir    ),
...
//**************new     *********************
 assign bpu_fixpc  = alu_cmt_pc;
  //**********************new start BPU***************************//
assign dec_rs1 = disp_alu_rs1;
assign bpu_fixwe_w = prmiss | prsuccess;
assign btb_fixwe_w = prmiss && cmt_isjump;

assign bpu_fixpht_w = (prsuccess && cmt_taken) ? ((bjp_o_cmt_pht < 2'b11) ? bjp_o_cmt_pht + 2'b1 : 2'b11)
                             : (prsuccess && !cmt_taken)  ? ((bjp_o_cmt_pht > 2'b0) ? bjp_o_cmt_pht - 2'b1 : 2'b0)
                             : (prmiss  &&  cmt_taken)    ? ((bjp_o_cmt_pht < 2'b11) ? bjp_o_cmt_pht + 2'b1 : 2'b11)
                             : (prmiss  && !cmt_taken)    ? ((bjp_o_cmt_pht > 2'b0) ? bjp_o_cmt_pht - 2'b1 : 2'b0)
                                                                     : bjp_o_cmt_pht ;

 assign bpu_fixjumpaddr = prsuccess ? bjp_o_cmt_takenpc : prmiss ? pipe_flush_pc : 1'b0;

   assign prdt_pc_add_op1 = (dec_bxx | dec_jal) ? dec_pc[`E203_PC_SIZE-1:0]
                         : dec_jalr ?  dec_rs1 : `E203_PC_SIZE'b0;  

  assign prdt_pc_add_op2 = dec_bjp_imm[`E203_PC_SIZE-1:0];  

  assign jump_addr = dec_bjp ? (prdt_pc_add_op1 + prdt_pc_add_op2) : `E203_PC_SIZE'b0;
//**********************new end BPU***************************//
...
wire [`E203_PC_SIZE-1:0]  jump_addr_real;
  e203_exu_commit u_e203_exu_commit(
    ...
    //****************new start***********************************//
    .i_bpu_taken_pc         (bjp_o_cmt_takenpc   ),
    .i_bpu_taken            (bjp_o_cmt_taken         ),
    .prmiss                 (prmiss              ),
    .prsuccess              (prsuccess           ),
    .cmt_isjump             (cmt_isjump          ),
    .cmt_taken              (cmt_taken           ),
    .dec_jalr              (dec_jalr),
      .i_rs1               (disp_alu_rs1     ),
      .jump_addr        (jump_addr),
      .dec_bjp            (dec_bjp),
      .jump_addr_real (jump_addr_real),
      .dec_bxx          (dec_bxx),
    //****************new end***********************************//
  ...
  assign dec2ifu_rdidx = dec_rdidx;  //用于反馈给BPU是否有rd的依�?
...
wire [`E203_PC_SIZE-1:0] ex_pc_r = alu_cmt_pc;
wire isjump_w = cmt_taken;
wire ex_inst1taken_r = i_bpu_taken;
wire [`E203_INSTR_SIZE-1:0] ex_inst_r = i_ir;
wire [`E203_PC_SIZE-1:0]  ex_inst1takenaddr_r = i_bpu_taken_pc;
wire [`E203_PC_SIZE-1:0]  jumpaddr_w = cmt_taken ?  jump_addr_real : (alu_cmt_pc+ 32'd4);
wire [`ysyx_22050058_BHRLEN-1:0] ex_inst1bhr_r = i_BHR;
...
reg [`ysyx_22050058_BHR_BUS] testghr;

always@(posedge clk or negedge rst_n)begin
    if (!rst_n) begin
        counterghrmiss <= 64'b0;
        jumpflag14<= 0;
    end else if((jumpflag13 != ex_pc_r)&&exisjump  &&({testghr[`ysyx_22050058_BHRLEN-1:0]} != {ex_inst1bhr_r[`ysyx_22050058_BHRLEN-1:0]}))begin
        //$display("testghr is %b and ghr is %b THIS PC is %x\n",{testghr[`ysyx_22050058_BHRLEN-1:0]},{ex_inst1bhr_r[`ysyx_22050058_BHRLEN-1:0]},ex_pc_w);
...
always@(posedge clk or negedge rst_n)begin
    if (!rst_n) begin
        testghr <= `ysyx_22050058_BHRLEN'b0;
        jumpflag15<= 0;
    end else if((jumpflag15 != ex_pc_r) &&exisjump )begin
        testghr <= {testghr[`ysyx_22050058_BHRLEN-2:0], isjump_w};
        jumpflag15<= ex_pc_r;
    end else begin
        jumpflag15<= ex_pc_r;
    end
...
always @(*) begin
    if (i_ir == 32'h81c50513) begin//81c50513
        $display("SIMU END \n JUMP INST is %8d\n GSHAREHIT is %8d\n BTBMISS   is %8d\n PRSUCCESS is %8d\n PRMISS    is %8d\n Miss Rate is %8d%%\n GSHAREHIT is %8d%%\n",
