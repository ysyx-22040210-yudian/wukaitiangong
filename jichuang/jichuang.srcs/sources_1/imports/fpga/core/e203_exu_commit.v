...
module e203_exu_commit(
  ...
  input  [`E203_PC_SIZE-1:0] jump_addr,
  input    dec_bjp,
  input   dec_bxx,
  output  [`E203_PC_SIZE-1:0]  jump_addr_real, 
  //********************************new end******************************** *********************
  ...
  //********************************new start******************************** *********************
  assign jump_addr_real = {`E203_PC_SIZE{alu_cmt_i_bjp}} & jump_addr     |
                                                                    {`E203_PC_SIZE{alu_cmt_i_ecall}} & csr_mtvec_r |
                                                                    {`E203_PC_SIZE{alu_cmt_i_mret}} & csr_epc_r     | `E203_PC_SIZE'b0;

  assign prmiss = dec_bxx ? (
          (i_bpu_taken ^ (
          (dec_bxx & alu_cmt_i_bjp_rslv)
        )) || 
        ((i_bpu_taken_pc != jump_addr_real) && (
          (dec_bxx & alu_cmt_i_bjp_rslv)
        ))
      ) : 1'b0;

  assign prsuccess = dec_bxx ? (
          (i_bpu_taken == (
          (dec_bxx & alu_cmt_i_bjp_rslv)
        )) && 
         ((i_bpu_taken_pc == jump_addr_real) &&
  ...
  assign cmt_isjump = (i_bpu_taken_pc != jump_addr_real) && (
         ( (dec_bxx & alu_cmt_i_bjp_rslv)
        ));
  /*
  assign prmiss = (
  ...
  //   If it is a DRET instruction, it is always jump 
       | alu_cmt_i_dret
          ...
          (i_bpu_taken ^ (
          (alu_cmt_i_bjp & alu_cmt_i_bjp_rslv)
    //   If it is a FenceI instruction, it is always Flush 
        | alu_cmt_i_fencei
    //   If it is a RET instruction, it is always jump 
        | alu_cmt_i_mret
...
*/   
  assign cmt_taken = (alu_cmt_i_bjp & alu_cmt_i_bjp_rslv)
    //   If it is a FenceI instruction, it is always Flush 
        | alu_cmt_i_fencei
    //   If it is a RET instruction, it is always jump 
        | alu_cmt_i_mret
