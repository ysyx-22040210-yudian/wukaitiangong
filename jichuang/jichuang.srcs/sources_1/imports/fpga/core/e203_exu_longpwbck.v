...
module e203_exu_longpwbck(
  ...
  `ifdef E203_HAS_NICE//{
  input  nice_longp_wbck_i_valid , //当nice core识别到nice指令的时候，为1
  ...
  // 长指令执行模块允许写回数据的时候，且其发送的itag实时与oitf的读指针比较，如果相同则允许写回
  ...
  wire wbck_sel_nice = nice_longp_wbck_i_valid & wbck_ready4nice; //nice对应的指令已经执行完毕，并且oitf的读指针与那个nice的itag相同，则选择写回nice指令的数据
  `endif//}
...
// 需要保证交付模块和最终写固仲裁模块同时能够接受数据
  assign wbck_i_ready = 
       (need_wbck ? longp_wbck_o_ready : 1'b1)
     & (need_excp ? longp_excp_o_ready : 1'b1);

//递给最终写回仲裁模块的握手请求
  assign longp_wbck_o_valid = need_wbck & wbck_i_valid & (need_excp ? longp_excp_o_ready : 1'b1); //如果需要异常，那么就要等待异常处理完成后才能写回
  //通给支付模块的握手请求
  assign longp_excp_o_valid = need_excp & wbck_i_valid & (need_wbck ? longp_wbck_o_ready : 1'b1); //如果需要写回，那么就要等待写回完成后才能处理异常
  ...
  assign oitf_ret_ena = wbck_i_valid & wbck_i_ready; //oitf读取wbck的数据后，就可以清空wbck的数据了
