...
module e203_exu_nice(
  ...
  input                          nice_rsp_multicyc_valid , //I: current insn is multi-cycle. 当nice指令被nice core识别到的时候为1
  ...
  wire   nice_req_valid_pos = nice_i_valid & nice_o_ready; //当派遣是的nice指令的时候，置1
  assign nice_req_valid = ~nice_i_xs_off &  nice_req_valid_pos; //当派遣是的nice指令的时候，置1
...
assign nice_o_itag_valid = fifo_o_vld & nice_rsp_multicyc_valid; //fifo中有数据，且nice core已经把对应的那条指令执行完毕了，准备写回
...
assign nice_o_itag = fifo_o_dat;
 ...
 sirv_gnrl_fifo # (
    ...
    .i_vld   (fifo_i_vld), //当有正在执行的nice指令的时候，就会往fifo中写入对应的itag，执行完那条指令之后，那个itag就会失效
    .i_rdy   (fifo_i_rdy),
    .i_dat   (fifo_i_dat),
    .o_vld   (fifo_o_vld), //判断是否有数据，如果为1代表fifo中有数据
    .o_rdy   (fifo_o_rdy),
    .o_dat   (fifo_o_dat),
    .clk     (clk  ),
    .rst_n   (rst_n)
