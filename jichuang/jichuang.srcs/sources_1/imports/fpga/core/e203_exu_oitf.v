...
//********************************new end *********************************--
 ...
 // wire [`E203_OITF_DEPTH-1:0] rdwen_r;//／／ 各表项中指令是否写回结果寄存器
  ...
  wire [`E203_PC_SIZE-1:0] pc_r[`E203_OITF_DEPTH-1:0];// II 各表项中指令的 PC
  ...
  if(`E203_OITF_DEPTH > 1) begin: depth_gt1//{
      wire alc_ptr_flg_r;
      wire alc_ptr_flg_nxt = ~alc_ptr_flg_r;  //when write point in max, to be change 
      wire alc_ptr_flg_ena = (alc_ptr_r == ($unsigned(`E203_OITF_DEPTH-1))) & alc_ptr_ena; //write point at max signal
      ...
      wire ret_ptr_flg_r;
      wire ret_ptr_flg_nxt = ~ret_ptr_flg_r;   //如果要增加oitf的表项，这里需要变成加法计算
 ...
 assign dis_ready = (~oitf_full);
  ...
  //／／每次分配一个表项时，且写指针与当前表 页编号一样 则将该表项的有效信号设置为高
        assign vld_set[i] = alc_ptr_ena & (alc_ptr_r == i);
        //／／ 每次移除 个表项时，旦 卖指针与当前表项编号一样，贝lj 将该表项的有效信号清除为低
        ...
        //／／其他的表项信息 均可视为该表项的载荷 Payload ）， 只需要在表项分配时写入，在 表项移除时无需清除
...
/*
  ...
  */
  ...
  endgenerate//}
  //**********************************new start *********************************--//
        assign rd_match_rs1idx[0] = vld_r[0] & rdwen_r[0] & disp_i_rs1en & (rdfpu_r[0] == disp_i_rs1fpu) & (rdidx_r0 == disp_i_rs1idx);
        assign rd_match_rs1idx[1] = vld_r[1] & rdwen_r[1] & disp_i_rs1en & (rdfpu_r[1] == disp_i_rs1fpu) & (rdidx_r1 == disp_i_rs1idx);
        assign rd_match_rs1idx[2] = vld_r[2] & rdwen_r[2] & disp_i_rs1en & (rdfpu_r[2] == disp_i_rs1fpu) & (rdidx_r2 == disp_i_rs1idx);
        assign rd_match_rs1idx[3] = vld_r[3] & rdwen_r[3] & disp_i_rs1en & (rdfpu_r[3] == disp_i_rs1fpu) & (rdidx_r3 == disp_i_rs1idx);

        assign rd_match_rs2idx[0] = vld_r[0] & rdwen_r[0] & disp_i_rs2en & (rdfpu_r[0] == disp_i_rs2fpu) & (rdidx_r0 == disp_i_rs2idx);
        assign rd_match_rs2idx[1] = vld_r[1] & rdwen_r[1] & disp_i_rs2en & (rdfpu_r[1] == disp_i_rs2fpu) & (rdidx_r1 == disp_i_rs2idx);
        assign rd_match_rs2idx[2] = vld_r[2] & rdwen_r[2] & disp_i_rs2en & (rdfpu_r[2] == disp_i_rs2fpu) & (rdidx_r2 == disp_i_rs2idx);
        assign rd_match_rs2idx[3] = vld_r[3] & rdwen_r[3] & disp_i_rs2en & (rdfpu_r[3] == disp_i_rs2fpu) & (rdidx_r3 == disp_i_rs2idx);

        assign rd_match_rs3idx[0] = vld_r[0] & rdwen_r[0] & disp_i_rs3en & (rdfpu_r[0] == disp_i_rs3fpu) & (rdidx_r0 == disp_i_rs3idx);
        assign rd_match_rs3idx[1] = vld_r[1] & rdwen_r[1] & disp_i_rs3en & (rdfpu_r[1] == disp_i_rs3fpu) & (rdidx_r1 == disp_i_rs3idx);
        assign rd_match_rs3idx[2] = vld_r[2] & rdwen_r[2] & disp_i_rs3en & (rdfpu_r[2] == disp_i_rs3fpu) & (rdidx_r2 == disp_i_rs3idx);
        assign rd_match_rs3idx[3] = vld_r[3] & rdwen_r[3] & disp_i_rs3en & (rdfpu_r[3] == disp_i_rs3fpu) & (rdidx_r3 == disp_i_rs3idx);

        assign rd_match_rdidx [0] = vld_r[0] & rdwen_r[0] & disp_i_rdwen & (rdfpu_r[0] == disp_i_rdfpu ) & (rdidx_r0 == disp_i_rdidx );
        assign rd_match_rdidx [1] = vld_r[1] & rdwen_r[1] & disp_i_rdwen & (rdfpu_r[1] == disp_i_rdfpu ) & (rdidx_r1 == disp_i_rdidx );
        assign rd_match_rdidx [2] = vld_r[2] & rdwen_r[2] & disp_i_rdwen & (rdfpu_r[2] == disp_i_rdfpu ) & (rdidx_r2 == disp_i_rdidx );
        assign rd_match_rdidx [3] = vld_r[3] & rdwen_r[3] & disp_i_rdwen & (rdfpu_r[3] == disp_i_rdfpu ) & (rdidx_r3 == disp_i_rdidx );

        assign   rdidx_r0 = rdidx_r[0];
        assign   rdidx_r1 = rdidx_r[1];
        assign   rdidx_r2 = rdidx_r[2];
        assign   rdidx_r3 = rdidx_r[3];
  //*****************************************new end ***********************//

    //*****************************new start**********************---
  /*sirv_gnrl_dffl #(`E203_RFIDX_WIDTH) rdidx_dfflrs0(vld_set[0], disp_i_rdidx, rdidx_r0, clk);
  sirv_gnrl_dffl #(`E203_RFIDX_WIDTH) rdidx_dfflrs1(vld_set[1], disp_i_rdidx, rdidx_r1, clk);
  sirv_gnrl_dffl #(`E203_RFIDX_WIDTH) rdidx_dfflrs2(vld_set[2], disp_i_rdidx, rdidx_r2, clk);
  sirv_gnrl_dffl #(`E203_RFIDX_WIDTH) rdidx_dfflrs3(vld_set[3], disp_i_rdidx, rdidx_r3, clk);*/
  //*******************************new end*********************//
