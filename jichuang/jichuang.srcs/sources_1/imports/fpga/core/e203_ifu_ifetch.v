...
module e203_ifu_ifetch(
  ...
  output ifu_o_pc_vld,
  //********************new**************************BPU********
  output [`BHR_WIDTH-1:0]        BHR_o_r,
  output [1:0]                   PHT_o_r,
  output [`E203_PC_SIZE-1:0]     bpu_taken_pc,
  output                         bpu_taken,

  input  [`BHR_WIDTH-1:0]        bpu_exfixbhr,
  input  [1:0]                   bpu_exfixpht,
  input  [`E203_PC_SIZE-1:0]     bpu_fixpc,
  input  [`E203_PC_SIZE-1:0]     bpu_fixjumpaddr,
  input                          bpu_exisjump,
  input  [2:0]                   ex_bjop           ,
  input                          bpu_fixwe_w       ,
  input                          btb_fixwe_w       ,
...
//********************************new end *********************************--
  ...
  wire ifu_req_hsked  = (ifu_req_valid & ifu_req_ready) ;//the hsked signed with ICB
  wire ifu_rsp_hsked  = (ifu_rsp_valid & ifu_rsp_ready) ;//the hsked signed with ICB
  wire ifu_ir_o_hsked = (ifu_o_valid & ifu_o_ready) ;  //the hsked signed with EXU's disp
  wire pipe_flush_hsked = pipe_flush_req & pipe_flush_ack; //the hsked signed with EXU's commit
 ...
 //    * Currently the flush can be accepted by IFU
  wire reset_req_r;
  wire reset_req_set = (~reset_req_r) & reset_flag_r;//当前没有复位请求，但产生了复位标志，要开始进行IFU复位
  wire reset_req_clr = reset_req_r & ifu_req_hsked;//正常工作，进行取指请求，所以复位要清除掉
  wire reset_req_ena = reset_req_set | reset_req_clr;//复位置位或者清零的时候，使能对复位请求的状态更新
  wire reset_req_nxt = reset_req_set | (~reset_req_clr); //set=1下个状态置位，set=1且clr=1时下个状态清零
  ...
  wire pc_newpend_r;
  wire ifu_ir_i_ready;
  assign ir_valid_set  = ifu_rsp_hsked & (~pipe_flush_req_real) & (~ifu_rsp_need_replay); //********************************KEY signal************************************************************************//
  ...
  wire prdt_taken;
  wire [`BHR_WIDTH-1:0]  bpu_bhr_o; 
  wire [1:0] bpu_pht_o; 
  wire ifu_prdt_taken_r;
  wire [`E203_PC_SIZE-1: 0]  bpu1hitaddr_o;
...
//***********new BPU******************************************--//
  sirv_gnrl_dfflr #(`BHR_WIDTH) ifu_bhr_ir_dfflr (ir_valid_set, bpu_bhr_o, BHR_o_r, clk, rst_n);
  sirv_gnrl_dfflr #(2) ifu_pht_ir_dfflr (ir_valid_set, bpu_pht_o, PHT_o_r, clk, rst_n);
  sirv_gnrl_dfflr #(`E203_PC_SIZE) ifu_bpupc_ir_dfflr (ir_valid_set, bpu1hitaddr_o, bpu_taken_pc, clk, rst_n);
  sirv_gnrl_dfflr #(1) ifu_bputaken_ir_dfflr (ir_valid_set, prdt_taken, bpu_taken, clk, rst_n);
//***************************new end**************************//
  ...
  .prdt_pc_add_op2          (prdt_pc_add_op2),
   ...
   // Interface with EX Stage
    .stall(!ir_valid_set),
    .flush(pipe_flush_hsked),
    // Interface with EX Stage
    .exisjump_i(bpu_exisjump),
    .gshare_prsuccessbutnotupdate_i(0),//if want tricky tuned bpu --->prsuccess&&(ex_inst1takenaddr_r=='h0)
    .bpu_prmiss_i(pipe_flush_hsked),
    .bpu_exrealbhr_i(bpu_exfixbhr),
    .bpu_exfixbhr_i(bpu_exfixbhr),
    // Interface with Commit Stage  
    .btb_fixwe_i       (btb_fixwe_w),
    .bpu_fixbhr_i      (bpu_exfixbhr),
    .bpu_fixpht_i      (bpu_exfixpht),
    .bpu_fixpc_i       (bpu_fixpc),
    .bpu_fixjumpaddr_i (bpu_fixjumpaddr),
    .bpu_fixwe_i       (bpu_fixwe_w),
    .btb_fixjumpop_i   (ex_bjop),
   // Interface with IF Stage
    .bpu_fetchpc_i            (pc_nxt  ), 
...
//********************new end********************************//
  ...
  wire [2:0] pc_incr_ofst = 3'd4;//minidec_rv32 ? 3'd4 : 3'd2;

  //wire bjp_req =  alu_cmt_pc < 32'h80000008 ? (bpu1hit_o & minidec_bjp) :  bpu1hit_o;//!!!!!!!!!!!!!!!!!!!!!!**********************可以结合译码优化******！！！！！！！！！
wire bjp_req = bpu1hit_o & minidec_bjp; //*****************************************************change
  ...
  wire [`E203_PC_SIZE-1:0] pc_add_op1 = 
                            ...
                            `endif//}
                               ...
                               ((bjp_req && alu_cmt_pc < 32'h80000008) || minidec_jalr) ? prdt_pc_add_op1    :
                              //bjp_req ? prdt_pc_add_op1 : /////****************************change
  ...
  wire [`E203_PC_SIZE-1:0] pc_add_op2 =  
                            ...
                            `endif//}
                              ...
                              ((bjp_req && alu_cmt_pc < 32'h80000008) || minidec_jalr) ? prdt_pc_add_op2    :
                             //bjp_req ? prdt_pc_add_op2    :  /////****************************change
  ...
  assign pc_nxt = 
               ...
               dly_pipe_flush_req ? {pc_r[`E203_PC_SIZE-1:1],1'b0} : //在冲刷流水线之后，但下一次的取指还没完成，这时候会延迟一个周期，保持在上一次的冲刷地址上
               (bjp_req &&  alu_cmt_pc >= 32'h80000008)? bpu1hitaddr_o :
