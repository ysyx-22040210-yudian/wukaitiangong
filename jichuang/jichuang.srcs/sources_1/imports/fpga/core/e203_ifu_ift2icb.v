...
// ===========================================================================
  ...
  //  
  // 检查当前访问的PC是否与上次访问的ICB地址相同的方案如下：
// * 我们只在以下情况下将其视为相同：当它是顺序指令获取请求，并且作为非对齐跨界（第一个16位和第二个16位跨界）时
// ** 如果ifetch请求是lane边界的开始，并且是顺序获取，
// 则：
// **** 如果上次预取时提前预取到了，则这次访问与上次访问的地址相同。否则不同。
// ** 如果ifetch请求不是lane边界的开始，并且是顺序获取，
// 则：
// **** 它必须访问与上次访问的地址相同。
// * 注意：所有其他非顺序情况（例如刷新、分支或重放）不视为此情况。
