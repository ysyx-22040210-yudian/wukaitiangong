...
generate //{
  if(ARBT_NUM == 1) begin:arbt_num_eq_1_gen// {
    ...
    if(ARBT_SCHEME == 0) begin:priorty_arbt//{ //固定优先级
      ...
      begin:priroty_grt_vec_gen
        if(i==0) begin: i_is_0
          ...
          assign i_bus_icb_cmd_grt_vec[i] =  ~(|i_bus_icb_cmd_valid[i-1:0]); //编号小的的优先
        end
        assign i_bus_icb_cmd_sel[i] = i_bus_icb_cmd_grt_vec[i] & i_bus_icb_cmd_valid[i]; //当高优先级的请求有效时，低优先级的请求不会被选中
      end//}
    end//}
    
    if(ARBT_SCHEME == 1) begin:rrobin_arbt//{ //轮询优先级
    ...
    always @ (*) begin : sel_o_apb_cmd_ready_PROC
      ...
      for(j = 0; j < ARBT_NUM; j = j+1) begin//{ //选择优先级最高的请求，其他请求不会被选中
      //i_bus_icb_cmd_sel向量中只有一个元素为1，其他为0
    ...
    always @ (*) begin : i_arbt_indic_id_PROC//{ //优先级编号
    ...
    if(ALLOW_0CYCL_RSP == 1) begin: allow_0rsp//{ //允许0周期响应
        assign rspid_fifo_bypass = rspid_fifo_empty & rspid_fifo_wen & rspid_fifo_ren; //在fifo为空的时候，且发出去的命令在一个周期内就完成了响应，则不需要写入fifo，直接bypass
        assign o_icb_rsp_port_id = rspid_fifo_empty ? rspid_fifo_wdat : rspid_fifo_rdat;//如果fifo为空，则直接使用写入fifo的数据，否则使用fifo输出的数据
    ...
    else begin: no_allow_0rsp//{ //不允许0周期响应
        assign rspid_fifo_bypass   = 1'b0;
        assign o_icb_rsp_port_id   = rspid_fifo_empty ? {ARBT_PTR_W{1'b0}} : rspid_fifo_rdat;
        assign o_icb_rsp_valid_pre = (~rspid_fifo_empty) & o_icb_rsp_valid;//如果fifo为空，则不允许响应
        assign o_icb_rsp_ready     = (~rspid_fifo_empty) & o_icb_rsp_ready_pre;//如果fifo为空，则不允许响应
    end

    assign rspid_fifo_i_valid = rspid_fifo_wen & (~rspid_fifo_bypass);//
    assign rspid_fifo_full    = (~rspid_fifo_i_ready);//fifo满的时候，i_ready为0
...
module sirv_gnrl_icb_buffer # (
  ...
  wire outs_cnt_inc = i_icb_cmd_valid & i_icb_cmd_ready;//当buffer请求握手成功时，计数器加1，对buf写数据
  wire outs_cnt_dec = i_icb_rsp_valid & i_icb_rsp_ready;//当buffer响应握手成功时，计数器减1，对buf读数据
  ...
  assign icb_buffer_active = i_icb_cmd_valid | (~(outs_cnt_r == {OUTS_CNT_W{1'b0}})); //当buffer请求握手成功时，或者计数器不为0时，buffer处于活动状态
