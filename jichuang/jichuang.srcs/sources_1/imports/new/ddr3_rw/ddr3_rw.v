...
module ddr3_rw(          
    ...
    output  [15:0]  app_wdf_mask         ,//屏蔽数据，控制信号
    output    reg  vsyncend    
    //output          wr_end_o      
    );
...
//reg           wr_end_d,wr_end_o; 
//wire          wr_end_r; 
// reg [1:0] vsyncend_cnt;
wire          rst_n;
...
//产生屏蔽标信号
assign app_wdf_mask =  vsyncend ==1'd0 ? 16'b10_10_10_10_10_10_10_10 : 16'b01_01_01_01_01_01_01_01 ;  //第一帧先写低位，第二帧写高位
//将数据读写地址赋给ddr地址
always @(*)  begin
    if(~rst_n)
        app_addr <= 0;
...
//帧计数器,没两帧才生标志位
 always @(posedge ui_clk or negedge rst_n)  begin
    if(~rst_n)                 
    vsyncend <= 1'b0;   
    else if (wr_end)  //需要写两次，在切换bank，在进行数据读取 
         vsyncend <= ~vsyncend;  
     else
        vsyncend <=  vsyncend ;
...
//    assign  wr_end_r = !wr_end_d &&  wr_end; 
...
//	.probe0(app_wdf_mask), // input wire [7:0]  probe0  
//	.probe1(vsyncend), // input wire [7:0]  probe1 
//	.probe2(wr_end), // input wire [0:0]  probe2 
//	.probe3(waddr_page) // input wire [0:0]  probe3 
